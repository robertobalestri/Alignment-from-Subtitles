...
INPUT_STRUCTURED_DIR = Path("subtitles_dataset_US_renamed_structured") # The Abbr/S##/E## folder
OUTPUT_EXPERIMENT_DIR = Path("output_experiment_gpt4_1_US")
CSV_OUTPUT_FILENAME = "gpt_4_1_racism_classification_results_US.csv"
LLM_TYPE_TO_USE = LLMType.GPT4_1
# --- End Configuration ---

# --- Load Base Prompt from file ---
PROMPT_FILE_PATH = Path(__file__).parent / "prompts" / "racism_experiment_prompt.txt"

try:
    with open(PROMPT_FILE_PATH, 'r', encoding='utf-8') as f:
        base_prompt_racism = f.read()
    logger.info(f"Prompt caricato da: {PROMPT_FILE_PATH}")
except FileNotFoundError:
    logger.error(f"File del prompt non trovato: {PROMPT_FILE_PATH}")
    base_prompt_racism = "FALLBACK_PROMPT_ERROR_LOADING_FILE" # o exit(1)
except Exception as e:
    logger.error(f"Errore nel caricamento del prompt da {PROMPT_FILE_PATH}: {e}")
    base_prompt_racism = "FALLBACK_PROMPT_ERROR_LOADING_FILE"
# --- End Prompt Loading ---
...
def classify_episode_json(
    ...
    try:
        ...
        prompt = base_prompt_racism + full_text
